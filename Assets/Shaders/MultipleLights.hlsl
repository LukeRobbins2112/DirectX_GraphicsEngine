...
{
    VS_OUTPUT output;
    output.PosMS = input.PosMS; // We pass along the raw model space position 
...
float4 PS(VS_OUTPUT input) : SV_Target
{
    ...
    float FogPercent = saturate((distToEye - FData.FogStart) / FData.FogRange);

    litColor =  litColor * mainTexture.Sample(aSampler, input.Tex);
    litColor = lerp(litColor, FData.FogColor, FogPercent);
    return litColor;
}
